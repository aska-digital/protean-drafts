...
Reusable renderer for the github-draft-review-html SOP: draw a self-contained,
GitHub-dark, single-file HTML draft for human review before posting to GitHub.

Maintained by the build role. Spec: skills/draft-review-html/SKILL.md
...
__version__ = "1.2.0"  # 1.2.0: --owners/--state, so external drafts carry the poster's handle
                        # instead of the internal protean chain (chrome only; body text unchanged)
...
<header class="gh-header">
  <div class="gh-header-inner">
    ...
    <div class="gh-right">
      ...
      <span class="gh-owners">{owner_chain}</span>
    </div>
  </div>
</header>
...
def main(argv=None) -> int:
    ...
    p.add_argument("--owners", default=DEFAULT_OWNERS,
                   help="header attribution: external drafts pass the poster's handle; "
                        "protean-internal drafts keep the default chain")
