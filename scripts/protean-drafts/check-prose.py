...
failed at least once.
    ...
    check-prose.py --denylist patterns.txt <file> [...]

Exit codes: 0 clean, 1 tells found, 3 usage/IO error.

Rules are from the external-writing-discipline and github-pr-audit skills.
A hit is a candidate to fix, not a verdict: a quote from someone else, a code block,
...
# Built-in patterns only: no private name is compiled into this file, and the
# machine-path shape is written so that no literal instance of it appears here.
# Deployment-specific names are supplied per run with --denylist <file>.
IDENTIFIER_PATTERNS = [
    r"/" + r"(?:Users|home|Volumes)" + r"/",
    r"~/\." + r"(?:hermes|config|local|cache)",
    r"\bteam[-_]skills\b",
    r"cache" + r"/" + r"delegation",
]
...
def load_denylist(path):
    ...
    for line in open(path, encoding="utf-8").read().splitlines():
        ...
        if not s or s.startswith("#"):
            continue
        out.append(s.split("=", 1)[1].strip() if "=" in s else s)
    return out


def check(name, raw, patterns=None):
    prose = strip_code(raw)
    patterns = IDENTIFIER_PATTERNS + list(patterns or [])
    findings = []
    ...
    for pat in patterns:
        for m in re.finditer(pat, raw, re.IGNORECASE):
            ...
            findings.append(f"{name}:{line_no}: internal identifier '{m.group(0)}' -> 'a neutral role name' / 'a relative path'")
...
def main(argv):
    ...
    deny = []
    if "--denylist" in argv:
        k = argv.index("--denylist")
        if k + 1 >= len(argv):
            print("check-prose: --denylist requires a path", file=sys.stderr)
            return 3
        deny = load_denylist(argv[k + 1])
